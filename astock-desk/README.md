...
- **牛熊裁判（原多空法庭）**：牛牛与熊熊轮流发言，一次只审一张卡，其中一条故意埋入逻辑缺陷，判决后揭晓并发放陷阱卡。
- **新手局**：AI 实时生成生活决策题（奶茶店加盟、健身年卡、理财课……），不用懂股票也能玩；模型未配置时回退到内置题库。
- **传唤证人**：每局一次，AI 实时给出一条不剧透的中立核验提示。
- **连胜与段位**：判对加分且连胜有加成，积分对应实习裁判→明眼人→火眼金睛→铁面判官；识破与踩过的陷阱进入「陷阱图鉴」。
